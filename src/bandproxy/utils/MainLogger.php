...
use LogLevel;
use bandproxy\Thread;
use bandproxy\Worker;
...
class MainLogger extends \AttachableThreadedLogger{
	...
	public function info($message, $name = "INFO"){
		$this->send($message, \LogLevel::INFO, $name, TextFormat::YELLOW);
	}
	...
	public function logException(\Throwable $e, $trace = null){
		...
		$errfile = \bandproxy\cleanPath($errfile);
		$this->log($type, get_class($e) . ": \"$errstr\" ($errno) in \"$errfile\" at line $errline");
		foreach(@\bandproxy\getTrace(1, $trace) as $i => $line){
			$this->debug($line);
		}
	}
    ...
    $message = TextFormat::toANSI(TextFormat::AQUA . "[ BandProxy - {$color}{$prefix}{$aqua}] -> {$color}{$message}");
